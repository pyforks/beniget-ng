...
This is soft fork of `serge-sans-paille/beniget <https://github.com/serge-sans-paille/beniget>`_ with the following changes: 

- Support trees generated by the standard library ``ast.parse`` function.
- Support the ``match`` statements (Python 3.10).
- Support the stub modules semantics, enabled when the filename ends with ``.pyi``.
- Support the ``try``-``except*`` clauses (Python 3.11).
- Support the type parameter syntax (Python 3.12). 
